...
int main(int argc, char *argv[]){
	...
	if (!main_flag && a && b && c){
		...
		print_array(size_a, a, argv[4], 'a', f);
		print_array(size_b, b, argv[4], 'b', f);
		print_array(size_c, c, argv[4], 'c', f);
		// counting numbers for each array
		am_a = count(a, size_a, mynumber, f);
		am_b = count(b, size_b, mynumber, f);
		am_c = count(c, size_c, mynumber, f);
	
		output_file = fopen(argv[4], "a");
		if (!fcos){
			fprintf(output_file, "Function is cos(x)\n");
		}
		else{
			fprintf(output_file, "Function is log(abs(x)) + sin(2x) - 1) \n");
		}
